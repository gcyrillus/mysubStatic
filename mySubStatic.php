	...
	class mySubStatic extends plxPlugin {
		...
		**/
		public function __construct($default_lang) {
			...
			$this->format = '				';
			$this->format .=$this->getParam('format')=='' ? '			<li class="#static_class #static_status" id="#static_id"><a href="#static_url" title="#static_name">#static_name</a></li>' : $this->getParam('format');
			$this->format .=PHP_EOL;
			$this->format_group = '			';
			$this->format_group.=$this->getParam('format_group')=='' ? '			<span class="#group_class #group_status">#group_name</span>' : $this->getParam('format_group');
			$this->format_group.=PHP_EOL;			
		}
		...
		public function IndexBegin() {
			...
			foreach ($plxShow->plxMotor->aStats as $k => $v) {
				if($v['group'] !=''  and $v['group'] !='home' and array_key_exists(trim(substr($v['group'],0,3)),$plxShow->plxMotor->aStats)  ) {
					...
					$substat =strtr($this->format_group, [
